...
export MONETACLI=${BUILDDIR}/qa/pull-tester/run-moneta-cli
export MONETAD=${REAL_MONETAD}
...
if [ "x${ENABLE_MONETAD}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and monetad must all be enabled"
fi
